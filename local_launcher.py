# ============================
# Ouroboros — Local launcher (runs on your own machine, no Google Colab)
# ============================
# Usage:
...
# ----------------------------
# 1) Secrets + runtime config (from environment / .env)
# ----------------------------
...
TOTAL_BUDGET_DEFAULT = _env("TOTAL_BUDGET", required=True)
GITHUB_TOKEN = _env("GITHUB_TOKEN", required=True)
GITHUB_USER = _env("GITHUB_USER", required=True)
GITHUB_REPO = _env("GITHUB_REPO", default="ouroboros", required=True)
...
ANTHROPIC_API_KEY = _env("ANTHROPIC_API_KEY", default="")
MAX_WORKERS = int(_env("OUROBOROS_MAX_WORKERS", default="5") or "5")
MODEL_MAIN = _env("OUROBOROS_MODEL", default="anthropic/claude-sonnet-4.6")
MODEL_CODE = _env("OUROBOROS_MODEL_CODE", default="anthropic/claude-sonnet-4.6")
MODEL_LIGHT = _env("OUROBOROS_MODEL_LIGHT", default=DEFAULT_LIGHT_MODEL)

BUDGET_REPORT_EVERY_MESSAGES = 10
SOFT_TIMEOUT_SEC = max(60, int(_env("OUROBOROS_SOFT_TIMEOUT_SEC", default="600") or "600"))
HARD_TIMEOUT_SEC = max(120, int(_env("OUROBOROS_HARD_TIMEOUT_SEC", default="1800") or "1800"))
DIAG_HEARTBEAT_SEC = _parse_int_cfg(
    _env("OUROBOROS_DIAG_HEARTBEAT_SEC", default="30"), default=30, minimum=0,
)
DIAG_SLOW_CYCLE_SEC = _parse_int_cfg(
    _env("OUROBOROS_DIAG_SLOW_CYCLE_SEC", default="20"), default=20, minimum=0,
)
...
# ----------------------------
DRIVE_ROOT = (pathlib.Path(__file__).parent / "data").resolve()
...
log.info("Ouroboros local launcher started. Send a message to your Telegram bot.")
